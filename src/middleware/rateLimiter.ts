...
export class RateLimiter {
  ...
  async waitForSlot(key: string): Promise<void> {
    while (!this.checkLimit(key)) {
      ...
      await this.sleep(this.getWaitTime(bucket));
    }
  ...
  private getBucket(key: string): TokenBucket {
    ...
    if (!bucket) {
      ...
      bucket = { tokens: limit, lastRefill: Date.now(), limit, windowMs: this.windowMs };
  ...
  private getLimit(key: string): number {
    if (key === 'global') return this.globalLimit;
  ...
  private getWaitTime(bucket: TokenBucket): number {
    return Math.ceil(bucket.windowMs / bucket.limit);
  }
